...
export interface VideoSuggestion extends Video {
  title?: string;
  photographer?: string;
}
...
export class PexelsAPI {
  ...
  async getVideoById(videoId: string): Promise<Video | null> {
    ...
    try {
      logger.debug({ videoId }, "Fetching video by ID from Pexels API");
      ...
      const response = await fetch(
        ...
        .then((res) => {
          if (!res.ok) {
            if (res.status === 404) {
              logger.warn({ videoId }, "Video not found in Pexels API");
              return null;
            }
            if (res.status === 401) {
              throw new Error(
        ...
        .catch((error: unknown) => {
          logger.error(error, "Error fetching video from Pexels API");
          throw error;
        });
      ...
      // Get the HD version if available
      ...
      logger.error(error, "Error fetching video by ID from Pexels API");
      throw error;
    }
  }

  private async _findVideos(
    searchTerm: string,
    ...
    logger.debug(
      ...
      "Searching for videos in Pexels API",
    );
    const headers = new Headers();
    headers.append("Authorization", this.API_KEY);
    const response = await fetch(
    ...
    // find all the videos that fit the criteria
          ...
          for (const file of video.video_files) {
            if (
              ...
              return {
                ...
                width: file.width,
                height: file.height,
                title: `${searchTerm} - ${video.user?.name || "Unknown"}`,
    ...
    return filteredVideos.slice(0, limit) as VideoSuggestion[];
  }
